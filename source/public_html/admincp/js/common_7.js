    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.success) {
				...
				$('#districts').append($("<option></option>").attr('value', 0).text('Chọn quận/huyện'));
                for (var i = 0; i < c; i++) {
...
function loadWard(district, selected) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.success) {
                var c = data.results.length
				$('#wards').append($("<option></option>").attr('value', 0).text('Chọn phường/xã'));
                for (var i = 0; i < c; i++) {
                    var row = data.results[i];
...
$('.citi_ajax_select').select2({
    ...
    multiple:true,
    ajax: {
        url: cmsUrl + "/districts/ajack-filter",
        dataType: 'json',
        type:'POST',
        quietMillis: 50,
        data: function (term) {
    ...
    initSelection: function(element, callback) {
        ...
        id = id.split(',');
        if (id!=="") {
            $.ajax(cmsUrl + "/districts/ajax-districts", {
                data: {
                    id:id
                },
                dataType: "json",
