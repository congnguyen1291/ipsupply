...
class WholesaleInvoiceTable extends AppTable
{
    public function fetchAll($where = array(), $order = array(), $intPage, $intPageSize)
    {
        ...
        $sql = new Sql($adapter);
        $select = $sql->select()->columns(array('*','total_products' => new Expression('count(wholesale_products.wholesale_products_id)')));
        $select->from('wholesale');
        $select->join('wholesale_products', 'wholesale_products.wholesale_id = wholesale.wholesale_id', array());
        $select->where(array(
            'website_id' => $this->getWebsiteId(),
        ));
        if(isset($where['products_id'])){
            $select->where(array(
                'wholesale_products.wholesale_products_id' => $where['products_id'],
            ));
        }
        if(isset($where['invoice_title'])){
            $invoice_title = $where['invoice_title'];
            $select->where->like('wholesale_title', "%{$invoice_title}%");
        }
        ...
        if(isset($where['date_create'])){
            ...
            if(count($between) == 2){
                $select->where("wholesale.date_create BETWEEN '{$between[0]}' AND '{$between[1]}'");
            }
        }
        if(isset($where['payment'])){
            $select->where(array(
                'wholesale.payment' => $where['payment'],
            ));
        }
        if(isset($where['delivery'])){
            $select->where(array(
                'wholesale.delivery' => $where['delivery'],
            ));
        }
        $select->where(array(
            'wholesale.is_delete' => 0,
        ));
        $select->where("wholesale.payment!='cancel'");
        if(!count($order)){
            $select->order(array(
                'wholesale.date_create' => 'ASC',
            ));
        }else{
            $select->order($order);
        }
        $select->group('wholesale.wholesale_id');
...
//            print_r($selectString);die;
    ...
    public function countAll($where = array()){
        ...
        */
        ...
        $adapter = $this->tableGateway->getAdapter();
        $sql = new Sql($adapter);
        $select = $sql->select()->columns(array('wholesale_id'));
        $select->from('wholesale');
        $select->join('wholesale_products', 'wholesale_products.wholesale_id = wholesale.wholesale_id', array());
        $select->where(array(
            'website_id' => $this->getWebsiteId()
    ...
    {
        try {
            $sql = "SELECT * FROM users WHERE website_id = {$this->getWebsiteId()}";
            $adapter = $this->tableGateway->getAdapter();
            $result = $adapter->query($sql, $adapter::QUERY_MODE_EXECUTE);
            return $result;
        } catch (\Exception $e) {
