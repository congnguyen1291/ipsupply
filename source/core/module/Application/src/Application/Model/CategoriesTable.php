...
class CategoriesTable extends AppTable {
	...
	public function getProductsIsGoingon($catid){
        ...
        if(!$results){
            ...
            $select->join('products_translate', 'products_translate.products_id=products.products_id',array('products_title','products_alias','products_description','products_longdescription_2','products_longdescription','bao_hanh', 'products_more', 'seo_keywords', 'seo_description', 'seo_title', 'promotion_description', 'promotion1_description', 'promotion2_description', 'promotion3_description', 'tags', 'language'));
            $select->join('products_type',new Expression('`products_type`.`products_id`=`products`.`products_id` AND `products_type`.`is_default` = 1'),array('products_type_id','t_price' => 'price', 't_price_sale'=>'price_sale', 't_quantity' => 'quantity', 't_is_available'=>'is_available'), 'left');
    ...
    public function getBannerForCat($catid){
        ...
        if(!$results){
            ...
            $catid = implode(',', $catid);
            $select->where("categories_banners.is_published=1 AND categories_banners.box_num = 1 AND categories_banners.categories_id IN({$catid}) AND categories.website_id= {$this->getWebsiteId()}");
            $select->order(array(
        ...
        if(!$results){
            try{
                ...
                if(!empty($rows) && !empty($rows[$categories_id])){
                    ...
                    while ( count($list) <= 5 && !empty($category) 
                            && !empty($category['parent_id']) && $category['parent_id'] >0
                            && !empty($rows[$category['parent_id']])) {
                        if( $category['parent_id'] == $category['categories_id'] )
                            break;
        ...
        if(!$results){
            try{
                ...
                $select->where(array(
                    'categories.is_published' => 1,
                    'categories.is_delete' => 0,
                    'categories.categories_id' => $cate_id,
                    'categories.website_id'=>$this->getWebsiteId(),
                    'categories_translate.language'=>$this->getLanguagesId()
                ));
                $selectString = $sql->getSqlStringForSqlObject($select);
        ...
        if(!$results){
			...
			if(count($cat_ids)>0){
				$select->where(array(
					'products.is_delete' => 0,
					'products.is_published' => 1,
					'products.categories_id' => $cat_ids,
                    'products.website_id'=>$this->getWebsiteId(),
                    'products_translate.language'=>$this->getLanguagesId()
				));
			}else{
    ...
    public function addSubscriptionUser($email, $cat)
    {
        ...
        $sql = "INSERT INTO `news_letters`(email,categories_id,news_letter_groups_id) (SELECT '{$email}','{$cat}', MIN(news_letter_groups_id) FROM news_letter_groups WHERE is_default=1) ON DUPLICATE KEY UPDATE `categories_id`='{$cat}' AND website_id ={$this->getWebsiteId()} ";// VALUES ('{$email}', '{$cat}') ON DUPLICATE KEY UPDATE `categories_id`='{$cat}';";
        try {
            $adapter = $this->tableGateway->getAdapter();
        ...
        if(!$results){
            ...
            try{
                ...
                $sql = "SELECT DISTINCT manufacturers.*
                        ...
                        WHERE products.is_published=1 AND products.is_delete=0 AND categories.is_published=1 AND products.website_id={$this->getWebsiteId()} AND categories.is_delete=0 AND categories.categories_id IN ({$cats}) AND manufacturers.is_published=1 AND manufacturers.is_delete=0
