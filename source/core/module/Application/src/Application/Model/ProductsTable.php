...
class ProductsTable extends AppTable{
	...
	public function getExtRequire($ext_require_id){
        ...
        if(!$results){
            ...
            $select->join('products_extensions_translate', 'products_extensions_translate.id=products_extensions.id', array('ext_name', 'ext_description', 'language') );
            $select->join('products', 'products.products_id=products_extensions.products_id', array() );
            $select->where(array(
                'products_extensions.id' => $ext_require_id,
                'products_extensions.ext_require' => 1,
                'products.website_id'=>$this->getWebsiteId(),
                'products_extensions_translate.language'=>$this->getLanguagesId()
            ));
    ...
    public function getTypeProduct($products_id){
        ...
        if(!$results){
            ...
            $sql = new Sql($adapter);
            $select = $sql->select();
            $select->columns(array('products_type_id', 'products_id', 'price', 'price_sale', 'quantity', 'is_available', 'thumb_image', 'is_default', 'thumb_image'));
            $select->from('products_type');
            $select->join('products_type_translate', 'products_type_translate.products_type_id=products_type.products_type_id', array('type_name', 'language') );
            $select->join ( 'products', 'products.products_id=products_type.products_id', array() );
            $select->where(array(
                'products.products_id' => $products_id,
                'products.website_id'=>$this->getWebsiteId(),
                'products_type_translate.language'=>$this->getLanguagesId()
            ));
	...
	public function getArticlesProduct($products_id){
        ...
        if(!$results) {
            ...
            $select->join('articles', 'products_articles.articles_id=articles.articles_id', array('articles_id','users_fullname','categories_articles_id','date_create','date_update','thumb_images'));
    ...
    public function getProductCateMore($cate_id, $id)
    {
        ...
        if(!$results) {
            try{
                ...
                $select->where->notIn('products.products_id', array($id));
            ...
            try{
    			...
    			if (isset($params['filter']) && $params['filter'] != '') {
    				switch ($params['filter']) {
    					...
    					case 'az' :
    						$select->order('products_translate.products_title ASC');
    						break;
    	
    					case 'za' :
    						$select->order('products_translate.products_title DESC');
    						break;
    				}
    			} else {
    			...
    			if (isset($params['fillmore']) && $params['fillmore'] != '') {
    				switch ($params['fillmore']) {
    					...
    					case 'deal':
    						$select->where(array(
    							'(products_translate.promotion_description IS NOT NULL OR
    							 products_translate.promotion1_description IS NOT NULL OR
    							 products_translate.promotion2_description IS NOT NULL OR
    							 products_translate.promotion3_description IS NOT NULL)'
    						));
    				}
    			}
    			$select->order('products.ordering ASC');
    ...
    public function getProductsIgnoreLocation($params)
    {
        $cache = $this->getServiceLocator()->get('cache');
        $stri_key = '';
        ...
        $keycache = md5($this->getNamspaceCached().':ProductsTable:getProductsIgnoreLocation('.$stri_key.')');
        ...
        if(!$results) {
            try{
                ...
                if ( !empty($params['country_id']) ) {
                    $select->where( " products.products_id NOT IN (
                            SELECT products_target.products_id 
                            FROM products_target
                            LEFT JOIN country ON FIND_IN_SET(country.id ,products_target.country_id)>0
                            WHERE (  country.id != '{$params['country_id']}'  AND FIND_IN_SET('{$params['country_id']}' ,products_target.country_id) <=0 )
                        ) " ) ;
                }
                ...
                if ( !empty($params['price']) ) {
                    ...
                    if (    count($price) == 2 
                            ...
                            && is_numeric($price[1])) {
                        $select->where('products.price_sale BETWEEN ' . $price[0] . ' AND ' . $price[1]);
                    }else if( count($price) == 1 && is_numeric($price[0]) ){
                        $select->where('products.price_sale >= ' . $price[0]);
                    }
                }
            ...
            try{
                ...
                if( $this->hasLocation() ){
                    ...
                    if ( !empty($params['cities']) ) {
                        $select->where( " products.products_id NOT IN (Select products_target.products_id FROM products_target LEFT JOIN cities ON FIND_IN_SET(cities.cities_id ,products_target.city_id)>0 WHERE ( cities.cities_id != {$params['cities']}  AND FIND_IN_SET({$params['cities']} ,products_target.city_id) <=0  )) " ) ;
                    }
                }
        ...
        if(!$results) {
            if(!$catids){
                ...
                $select->order(array(
                    ...
                    'products.number_views' => 'DESC',
                    'products.number_like' => 'DESC',
                ));
				$select->group('products.products_id');
                $select->limit(3);
                $selectString = $sql->getSqlStringForSqlObject($select);
    ...
    public function countTotalProduct($cate_id, $params)
    {
        ...
        if(!$results) {
            try{
                ...
                $select = $sql->select()->columns(array('total' => new Expression("count(DISTINCT products.products_id)")));
                $select->from('products');
                $select->join('products_translate', 'products_translate.products_id=products.products_id',array());
                $select->join('products_type',new Expression('`products_type`.`products_id`=`products`.`products_id` AND `products_type`.`is_default` = 1'),array(), 'left');
                $select->join('products_type_translate', new Expression("products_type_translate.products_type_id=products_type.products_type_id AND products_type_translate.language={$this->getLanguagesId()}" ), array(),'left');
				$select->join('products_category', 'products_category.products_id=products.products_id', array());
                $select->where(array(
                    'products.is_published' => 1,
                    'products.is_delete' => 0,
    ...
    public function getTotalProductIgnoreLocation($params)
    {
        $cache = $this->getServiceLocator()->get('cache');
        $stri_key = '';
        if(is_array($params)){
        ...
        $key = md5($this->getNamspaceCached().':ProductsTable:getTotalProductIgnoreLocation('.$stri_key.')');
        $results = $cache->getItem($key);
        if(!$results) {
            try{
                $adapter = $this->tableGateway->getAdapter();
    ...
    public function insertKeyword($keyword)
    {
        try {
            ...
            if (trim($keyword)) {
                ...
                $sql = "INSERT INTO search_keywords(`website_id`,`keyword_title`,`total_search`) VALUES (".$this->getWebsiteId().",'{$keyword}', 1) ON DUPLICATE KEY UPDATE total_search=total_search+1";
            ...
            try{
                ...
                if (is_array($cate_id)) {
                    foreach ($cate_id as $id) {
                        $sql[] = "(SELECT products.products_id, products.products_code, products.categories_id, products.manufacturers_id, products.users_id, products.website_id, products.transportation_id, products.users_fullname, products.is_published,products.is_delete, products.is_new, products.is_hot,products.is_available, products.is_goingon, products.is_sellonline, products.tra_gop, products.date_create, products.date_update, products.hide_price, products.wholesale, products.price, products.price_sale, products.ordering, products.quantity, products.thumb_image, products.list_thumb_image, products.number_views, products.vat, products.rating, products.number_like, products.total_sale, products.convert_search, products.is_viewed, products.position_view, products.youtube_video, products.price_total, products.url_crawl, products.convert_sitemap, products.convert_images, products.tags, products.type_view, products.publisher_id,`products_translate`.`products_title`, `products_translate`.`products_alias`, `products_translate`.`products_description`, `products_translate`.`products_longdescription_2`, `products_translate`.`products_longdescription`, `products_translate`.`bao_hanh`, `products_translate`.`products_more`, `products_translate`.`seo_keywords`, `products_translate`.`seo_description`, `products_translate`.`seo_title`, `products_translate`.`promotion_description`, `products_translate`.`promotion1_description`, `products_translate`.`promotion2_description`, `products_translate`.`promotion3_description`, `products_translate`.'tags`, `products_translate`.`language`, `categories`.`categories_title` AS `categories_title`, count(comments_id) as total_review, (SELECT SUM(`products_extensions`.`price`*`products_extensions`.`quantity`) FROM `products_extensions` WHERE `products_extensions`.`products_id` = `products`.`products_id` AND `products_extensions`.`is_always` = 1 ) AS total_price_extention
                                FROM `products`
                                INNER JOIN `products_translate` ON products_translate.products_id=products.products_id
                                ...
                                WHERE `products`.`is_published` = '1' AND `products`.`is_delete` = '0' AND products.website_id={$this->getWebsiteId()} AND products_translate.language = {$this->getLanguagesId()}
    ...
    public function getConcernProduct()
    {
        ...
        if(!$results) {
			...
			$select->where(array(
				'products.is_published' => 1,
				'products.is_delete' => 0,
                'products.products.website_id'=>$this->getWebsiteId(),
                'products_translate.language'=>$this->getLanguagesId()
			));
    ...
    public function getProductsWithType($id, $product_type)
    {
        ...
        if(!$results) {
            ...
            $select->from('products');
            $select->join('products_translate', 'products_translate.products_id=products.products_id',array('products_title','products_alias','products_description','products_longdescription_2','products_longdescription','bao_hanh', 'products_more', 'seo_keywords', 'seo_description', 'seo_title', 'promotion_description', 'promotion1_description', 'promotion2_description', 'promotion3_description', 'tags', 'language'));
            $select->join('products_type',new Expression('`products_type`.`products_id`=`products`.`products_id` AND `products_type`.`products_type_id` = '.$product_type),array('products_type_id','t_price' => 'price', 't_price_sale'=>'price_sale', 't_quantity' => 'quantity', 't_is_available'=>'is_available', 't_thumb_image'=>'thumb_image'));
            $select->join('products_type_translate', new Expression("products_type_translate.products_type_id=products_type.products_type_id AND products_type_translate.language={$this->getLanguagesId()}" ), array('type_name', 'language'));
            $select->join('comments', 'comments.comments_product=products.products_id', array('total_review' => new Expression('count(comments_id)')), 'left');
            $select->join('manufacturers', 'products.manufacturers_id=manufacturers.manufacturers_id', array('manufacturers_name', 'manufacturers_id'));
            $select->where(array(
                'products.is_published' => 1,
    ...
    public function getImages($id)
    {
        ...
        if(!$results) {
            ...
            $sql = new Sql($adapter);
            $select = $sql->select();
            $select->columns( array('id', 'products_id', 'images', 'thumb_image' => 'images', 'is_published', 'date_create', 'ordering', 'check_convert') );
            ...
            $select->join('products_type',new Expression('`products_type`.`products_id`=`products`.`products_id` AND `products_type`.`is_default` = 1'),array('products_type_id', 't_thumb_image'=>'thumb_image'), 'left');
            $select->where(array(
                'products.is_published' => 1,
                'products.products_id' => $id,
                'products.website_id'=>$this->getWebsiteId()
    ...
    public function getFeature($id)
    {
        ...
        if(!$results) {
            ...
            $select->where(array(
                ...
                'feature.website_id'=>$this->getWebsiteId()
            ));
            try{
                $selectString = $sql->getSqlStringForSqlObject($select);
                $results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE);
    ...
    public function getHotDeal($where = array())
    {
        ...
        $results = $cache->getItem($key);
        if(!$results) {
            $where[] = "products.website_id=".$this->getWebsiteId()." AND (
                        products_translate.promotion_description IS NOT NULL ||
                        products_translate.promotion1_description IS NOT NULL ||
                        products_translate.promotion2_description IS NOT NULL ||
                        products_translate.promotion3_description IS NOT NULL)";
            $where = implode(' AND ', $where);
            $sql = "SELECT `products`.`products_id`, `products`.`products_code`, `products`.`categories_id`, `products`.`manufacturers_id`, `products`.`users_id`, `products`.`website_id`, `products`.`transportation_id`, `products`.`users_fullname`, `products`.`is_published`,`products`.`is_delete`, `products`.`is_new`, `products`.`is_hot`,`products`.`is_available`, `products`.`is_goingon`, `products`.`is_sellonline`, `products`.`tra_gop`, `products`.`date_create`, `products`.`date_update`, `products`.`hide_price`, `products`.`wholesale`, `products`.`price`, `products`.`price_sale`, `products`.`ordering`, `products`.`quantity`, `products`.`thumb_image`, `products`.`list_thumb_image`, `products`.`number_views`, `products`.`vat`, `products`.`rating`, `products`.`number_like`, `products`.`total_sale`, `products`.`convert_search`, `products`.`is_viewed`, `products`.`position_view`, `products`.`youtube_video`, `products`.`price_total`, `products`.`url_crawl`, `products`.`convert_sitemap`, `products`.`convert_images`, `products`.`tags`, `products`.`type_view`, `products`.`publisher_id`,`products_translate`.`products_title`, `products_translate`.`products_alias`, `products_translate`.`products_description`, `products_translate`.`products_longdescription_2`, `products_translate`.`products_longdescription`, `products_translate`.`bao_hanh`, `products_translate`.`products_more`, `products_translate`.`seo_keywords`, `products_translate`.`seo_description`, `products_translate`.`seo_title`, `products_translate`.`promotion_description`, `products_translate`.`promotion1_description`, `products_translate`.`promotion2_description`, `products_translate`.`promotion3_description`, `products_translate`.'tags`, `products_translate`.`language`, products_type.products_type_id, products_type.price AS t_price, products_type.price_sale AS t_price_sale, products_type.quantity AS t_quantity, products_type.is_available AS t_is_available, products_type_translate.type_name, thumb_image AS t_thumb_image FROM products INNER JOIN `products_translate` ON products_translate.products_id=products.products_id LEFT JOIN products_type ON (`products_type`.`products_id`=`products`.`products_id` AND `products_type`.`is_default` = 1) LEFT JOIN products_type_translate ON (products_type_translate.products_type_id=products_type.products_type_id AND products_type_translate.language={$this->getLanguagesId()}) WHERE {$where} ORDER BY date_update DESC";
            try {
                $adapter = $this->tableGateway->getAdapter();
                $results = $adapter->query($sql, $adapter::QUERY_MODE_EXECUTE);
                $results = $results->toArray();
    ...
    public function updateAddsearch($products_id)
    {
        try {
            $adapter = $this->tableGateway->getAdapter();
            $sql = "update products set convert_search=1 WHERE products_id={$products_id} AND website_id=".$this->getWebsiteId();
            $adapter->query($sql, $adapter::QUERY_MODE_EXECUTE);
        } catch (\Exception $ex) {
            return FALSE;
        }
    ...
    public function getByManus($manuid, $limit){
        ...
        if(!$results) {
            ...
            $select->join('comments', 'comments.comments_product=products.products_id', array('total_review' => new Expression("count(DISTINCT comments.comments_id)")), 'left');
            $select->where(array(
                'products.manufacturers_id' => $manuid,
                'products.is_published' => 1,
                'products.is_delete' => 0,
                'products.website_id'=>$this->getWebsiteId(),
                'products_translate.language'=>$this->getLanguagesId()
    ...
    public function getRecommendProductByCatId($catId, $limit)
    {
        ...
        if(!$results) {
            try {
                ...
                $select->join('categories', 'categories_recommend.to_categories_id=categories.categories_id', array());
                $select->join('categories_translate', 'categories_translate.categories_id=categories.categories_id',array('categories_title','categories_alias'));
                $select->where(array(
                    'from_categories_id' => $catId,
                    'categories.website_id'=>$this->getWebsiteId(),
                    'categories_translate.language'=>$this->getLanguagesId()
                ));
                $selectString = $sql->getSqlStringForSqlObject($select);
                $results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE);
                $cat_data = array();
                foreach ($results as $cat) {
                    ...
                    $select->join('products_type_translate', new Expression("products_type_translate.products_type_id=products_type.products_type_id AND products_type_translate.language={$this->getLanguagesId()}" ), array('type_name', 'language'),'left');
                    $select->where(array(
                        'products.categories_id' => $cat->categories_id,
                        'products.is_published' => 1,
                        'products.is_delete' => 0,
                        'products.website_id'=>$this->getWebsiteId(),
                        'products_translate.language'=>$this->getLanguagesId()
    ...
    public function getRecommendProductById($products_id, $offset = 0, $limit = 5)
    {
        ...
        if(!$results) {
            try {
                ...
                $select->join('products', 'products_recommend.to_products_id=products.products_id', array('products_id', 'products_code','categories_id','manufacturers_id','users_id','website_id','transportation_id','users_fullname','is_published', 'is_delete', 'is_new', 'is_hot', 'is_available', 'is_goingon', 'is_sellonline', 'tra_gop', 'date_create', 'date_update', 'hide_price', 'wholesale', 'price', 'price_sale', 'ordering', 'quantity', 'thumb_image', 'list_thumb_image', 'number_views', 'vat', 'rating', 'number_like', 'total_sale', 'convert_search', 'is_viewed', 'position_view', 'youtube_video', 'price_total', 'url_crawl', 'convert_sitemap', 'convert_images', 'tags', 'type_view', 'publisher_id','title_extention_always'=> new Expression('(SELECT GROUP_CONCAT(`products_extensions`.`ext_name`) FROM `products_extensions` WHERE `products_extensions`.`products_id` = `products`.`products_id` AND `products_extensions`.`is_always` = 1 )'),'total_price_extention'=> new Expression('(SELECT SUM(`products_extensions`.`price`*`products_extensions`.`quantity`) FROM `products_extensions` WHERE `products_extensions`.`products_id` = `products`.`products_id` AND `products_extensions`.`is_always` = 1 )')));
                $select->join('products_translate', 'products_translate.products_id=products.products_id',array('products_title','products_alias','products_description','products_longdescription_2','products_longdescription','bao_hanh', 'products_more', 'seo_keywords', 'seo_description', 'seo_title', 'promotion_description', 'promotion1_description', 'promotion2_description', 'promotion3_description', 'tags', 'language'));
                $select->join('products_type',new Expression('`products_type`.`products_id`=`products`.`products_id` AND `products_type`.`is_default` = 1'),array('products_type_id','t_price' => 'price', 't_price_sale'=>'price_sale', 't_quantity' => 'quantity', 't_is_available'=>'is_available', 't_thumb_image'=>'thumb_image'), 'left');
                $select->join('products_type_translate', new Expression("products_type_translate.products_type_id=products_type.products_type_id AND products_type_translate.language={$this->getLanguagesId()}" ), array('type_name', 'language'),'left');
    ...
    public function getProductNearPrice($catId, $products_id, $price_sale, $min, $max, $offset = 0, $limit = 5)
    {
        ...
        if(!$results) {
            ...
            try {
                $sql = new Sql($adapter); 
                $select = $sql->select()->columns(array('products_id', 'products_code','categories_id','manufacturers_id','users_id','website_id','transportation_id','users_fullname','is_published', 'is_delete', 'is_new', 'is_hot', 'is_available', 'is_goingon', 'is_sellonline', 'tra_gop', 'date_create', 'date_update', 'hide_price', 'wholesale', 'price', 'price_sale', 'ordering', 'quantity', 'thumb_image', 'list_thumb_image', 'number_views', 'vat', 'rating', 'number_like', 'total_sale', 'convert_search', 'is_viewed', 'position_view', 'youtube_video', 'price_total', 'url_crawl', 'convert_sitemap', 'convert_images', 'tags', 'type_view', 'publisher_id',
                    ...
                    'total_price_extention'=> new Expression('(SELECT SUM(`products_extensions`.`price`*`products_extensions`.`quantity`) FROM `products_extensions` WHERE `products_extensions`.`products_id` = `products`.`products_id` AND `products_extensions`.`is_always` = 1 )')));
                $select->from('products');
                $select->join('products_translate', 'products_translate.products_id=products.products_id',array('products_title','products_alias','products_description','products_longdescription_2','products_longdescription','bao_hanh', 'products_more', 'seo_keywords', 'seo_description', 'seo_title', 'promotion_description', 'promotion1_description', 'promotion2_description', 'promotion3_description', 'tags', 'language'));
                $select->join('products_type',new Expression('`products_type`.`products_id`=`products`.`products_id` AND `products_type`.`is_default` = 1'),array('products_type_id','t_price' => 'price', 't_price_sale'=>'price_sale', 't_quantity' => 'quantity', 't_is_available'=>'is_available', 't_thumb_image'=>'thumb_image'), 'left');
                $select->join('products_type_translate', new Expression("products_type_translate.products_type_id=products_type.products_type_id AND products_type_translate.language={$this->getLanguagesId()}" ), array('type_name', 'language'),'left');
                $select->where(" ((products.price_sale BETWEEN {$min} AND {$max}) OR (products_type.price_sale BETWEEN {$min} AND {$max})) AND products.products_id != {$products_id} AND products.categories_id={$catId} AND products.is_published=1 AND products.is_delete=0 AND products.website_id={$this->getWebsiteId()} AND products_translate.language={$this->getLanguagesId()} ");
                $select->order(array(
                    'distance_price' => 'ASC',
                ));
                if( $this->hasLocation() ){
