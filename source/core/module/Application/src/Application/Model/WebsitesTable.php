...
class WebsitesTable extends AppTable {
    ...
    public function getWebsite($domain){
        ...
        if(!$results){
            ...
            $select->join('template', 'websites.template_id=template.template_id',array('categories_template_id','template_name', 'template_dir', 'template_folder', 'files_js', 'files_css'), 'left');
            $select->join('websites_app_facebook', 'websites.website_id=websites_app_facebook.website_id',array('default_access_token' ,'long_access_token', 'default_page_access_token', 'long_page_access_token', 'page_id', 'page_tab', 'date_access_token', 'date_sync', 'app_facebook_has_configed'), 'left');
	 ...
	 public function getWebsiteInfo(){
        ...
        if(!$results){
            ...
            try{
                ...
                $results = (is_array($this->getWebsiteId()) ? $resultSet->toArray() : $resultSet->current());
                $cache->setItem($key, $results);
            }catch(\Exception $ex){
                return array();
            }
    ...
    public function updateAccessTokenFacebook( $website_id, $domain, $default_access_token, $long_access_token = '', $default_page_access_token = '', $long_page_access_token = '' , $page_id = '' , $page_tab = '' ){
        if( !empty($website_id) && !empty($default_access_token) ){
            try{
                $adapter = $this->tableGateway->getAdapter();
                $sql = new Sql($adapter);
                $delete = $sql->delete('websites_app_facebook')->where("website_id = ".$website_id);
                $deleteString = $sql->getSqlStringForSqlObject($delete);
                $adapter->query($deleteString, $adapter::QUERY_MODE_EXECUTE);

                $sql = new Sql($adapter);
                $insert = $sql->insert('websites_app_facebook');
                $insert->columns(array('website_id', 'default_access_token', 'long_access_token', 'default_page_access_token', 'long_page_access_token', 'page_id', 'page_tab', 'date_access_token', 'date_sync'));
                $insert->values(array(
                    'website_id' => $website_id,
                    'default_access_token' => $default_access_token,
                    'long_access_token' => $long_access_token,
                    'default_page_access_token' => $default_page_access_token,
                    'long_page_access_token' => $long_page_access_token,
                    'page_id' => $page_id,
                    'page_tab' => $page_tab,
                    'app_facebook_has_configed' => 0,
                    'date_access_token' => Date('Y-m-d H:m:i'),
                    'date_sync' => '',
                ));
    ...
    public function updateDateSyncFacebook(){
        try{
            $adapter = $this->tableGateway->getAdapter();
            $sql = new Sql($adapter);
            $sql    = new Sql( $this->adapter );
            $update = $sql->update();
            $update->table( 'websites_app_facebook' );
            $update->set( array('date_sync' => Date('Y-m-d H:m:i') ) );
            $update->where( array( 'website_id' => $_SESSION['website_id'] ) );
    ...
    public function updateHasConfigFacebook(){
        try{
            $adapter = $this->tableGateway->getAdapter();
            $sql = new Sql($adapter);
            $sql    = new Sql( $this->adapter );
            $update = $sql->update();
            $update->table( 'websites_app_facebook' );
            $update->set( array('app_facebook_has_configed' =>    1 ) );
            $update->where( array( 'website_id' => $_SESSION['website_id'] ) );
    ...
    public function updatePageIDFacebook( $page_id ){
        try{
            $adapter = $this->tableGateway->getAdapter();
            $sql = new Sql($adapter);
            $sql    = new Sql( $this->adapter );
            $update = $sql->update();
            $update->table( 'websites_app_facebook' );
            $update->set( array('page_id' =>    $page_id, 'app_facebook_has_configed' =>    1 ) );
            $update->where( array( 'website_id' => $_SESSION['website_id'] ) );
    ...
    public function removeAccessTokenFacebook(){
        try{
            $adapter = $this->tableGateway->getAdapter();
            $sql = new Sql($adapter);
            $delete = $sql->delete('websites_app_facebook')->where("website_id = ".$_SESSION['website_id']);
            $deleteString = $sql->getSqlStringForSqlObject($delete);
            $adapter->query($deleteString, $adapter::QUERY_MODE_EXECUTE);
    ...
    public function removeCacheWebsiteDomain($domain){
        $cache = $this->getServiceLocator()->get('cache');
        $key = md5($this->getNamspaceCached().':Websites:getwebsite('.$domain.')');
        $results = $cache->getItem($key);
        $cache->setItem($key, NULL);
